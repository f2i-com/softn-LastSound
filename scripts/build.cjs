...
'use strict';
/**
 * Build dist/LastSound3D.softn from bundle/, with checksums and release notes.
 *
...
try {
  ...
  const out = path.join(distDir, 'LastSound3D.softn');
  fs.writeFileSync(out, bytes);
  const sha256 = crypto.createHash('sha256').update(bytes).digest('hex');
  fs.writeFileSync(path.join(distDir, 'SHA256SUMS.txt'), `${sha256}  LastSound3D.softn\n`);
  const count = (dir) => (fs.existsSync(path.join(bundleDir, 'assets', dir)) ? fs.readdirSync(path.join(bundleDir, 'assets', dir)).length : 0);
  const notes = [
    `# LAST SOUND: Below the Call v${manifest.version}`,
    '',
    manifest.description,
    '',
    `- Bundle: \`LastSound3D.softn\`, ${(bytes.length / 1048576).toFixed(2)} MB, ${entries} entries`,
    `- SHA-256: \`${sha256}\``,
    `- Voice lines: ${count('voice')} · sound effects: ${count('sfx')} · survey photographs and decals: ${count('photos') + count('decals')}`,
    '',
